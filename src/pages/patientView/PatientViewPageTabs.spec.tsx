...
import { WsiHierarchyFetchError } from 'shared/components/wsiViewer/wsiHierarchyFetchCache';
...
describe('SummaryTimelineSection', () => {
    ...
    it('does not request the WSI hierarchy when WSI is disabled', () => {
        mockServerConfig.msk_wsi_tile_server_url = '';
        mockUsePathologyAugmentedClinicalEvents.mockReturnValue({
            events: [],
            eventsSignature: 'events',
            hierarchyLoadState: 'idle',
            hierarchyError: null,
            undatedPathologySlideCount: 0,
        });
        ...
        expect(
            mockUsePathologyAugmentedClinicalEvents.mock.calls[0][0]
        ).toEqual(expect.objectContaining({ includeUndatedPathology: false }));
        mockServerConfig.msk_wsi_tile_server_url = 'https://slides.example.com';
    });

    it('does not show an error banner when the configured WSI hierarchy is absent', () => {
        mockUsePathologyAugmentedClinicalEvents.mockReturnValue({
            events: [],
            eventsSignature: 'events',
            hierarchyLoadState: 'absent',
            hierarchyError: null,
            undatedPathologySlideCount: 0,
        });
    ...
    it.each([401, 403, 500, 503])(
        'shows an error banner for HTTP %i hierarchy failures',
        status => {
            mockUsePathologyAugmentedClinicalEvents.mockReturnValue({
                events: [],
                eventsSignature: 'events',
                hierarchyLoadState: 'error',
                hierarchyError: new WsiHierarchyFetchError(status),
                undatedPathologySlideCount: undefined,
            });
...
describe('tabs', () => {
    ...
    it('advertises WSI slides from v3 count attributes without legacy slide IDs', () => {
        const pageComponent = makePageComponent({
            pageStore: {
                clinicalEvents: {
                    isComplete: true,
