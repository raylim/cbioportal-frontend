...
export type PathologyHierarchyLoadState =
    | 'idle'
    | 'loading'
    | 'ready'
    | 'absent'
...
export function usePathologyAugmentedClinicalEventsState({
    ...
    const [hierarchyLoadState, setHierarchyLoadState] = useState<
        PathologyHierarchyLoadState
    >('idle');
        ...
        void fetchPatientHierarchyReadOnly(hierarchyUrl, controller.signal)
            ...
            .catch(error => {
                if (!cancelled && error?.name !== 'AbortError') {
                    if (
                        error instanceof WsiHierarchyFetchError &&
                        error.status === 404
                    ) {
                        setHierarchyLoadState('absent');
                        setHierarchyError(null);
                    } else {
    ...
    const undatedPathologySlideCount = !includeUndatedPathology
        ? 0
