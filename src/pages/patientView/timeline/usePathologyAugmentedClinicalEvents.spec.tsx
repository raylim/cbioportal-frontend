...
describe('usePathologyAugmentedClinicalEventsState', () => {
    beforeEach(() => {
        // The hook builds the hierarchy URL when the undated fallback is
        // enabled. Keep this child test independent from the browser config
        // that is injected by the running application.
    ...
    it('treats a missing hierarchy as absent without counting slides', async () => {
        let renderedState:
            | ReturnType<typeof usePathologyAugmentedClinicalEventsState>
            | undefined;
        const originalFetch = global.fetch;
        ...
        try {
            ...
            expect(renderedState?.hierarchyLoadState).toBe('absent');
            expect(renderedState?.hierarchyError).toBeNull();
            expect(renderedState?.undatedPathologySlideCount).toBe(0);
        } finally {
            global.fetch = originalFetch;
        }
    });
    ...
    it.each([401, 403, 500, 503])(
        'retains HTTP %i hierarchy failures',
        async status => {
            let renderedState:
                | ReturnType<typeof usePathologyAugmentedClinicalEventsState>
                | undefined;
            ...
            try {
                ...
                expect(renderedState?.hierarchyLoadState).toBe('error');
                expect(renderedState?.hierarchyError).toEqual(
